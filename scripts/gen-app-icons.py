...
def symbolic(alert):
    ...
    out = [
        ...
        "  SPDX-License-Identifier: GPL-3.0-or-later",
        "",
        "  Drawn on a 16 grid: at tray sizes the stem is 1.75px and the mark fills",
        "  the box. Generated by scripts/gen-app-icons.py.",
        "-->",
...
def colour(alert, year="2021", holders="Guy Boldon and contributors"):
    heads, stems = mark(BAND_COLOUR, scale=16.0, offset=0.0, fmt=".3f")
    ...
    out.append("  </g>")
    if alert:
        out.append(_badge_circle(16.0, 0.0).rstrip("\n"))
    out.append("</svg>")
    return "\n".join(out) + "\n"
...
def main():
    ...
    art = render(logo, 360)
    canvas.alpha_composite(art, (76, 76))
