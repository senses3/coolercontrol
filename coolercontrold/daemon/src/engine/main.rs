...
use crate::hardware_probe::{ProbeConditions, ProbeHost};
...
/// Rounds a reported duty to the whole percent the probe plans against.
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
fn duty_from_percent(percent: f64) -> Duty {
    percent.round().clamp(0.0, 100.0) as Duty
}

impl Engine {
    /// The channel's control capability and its latest reported state.
    ///
    /// `None` when the channel is unknown, or when a channel we *can* write to
    /// reported no duty: planning from a guessed duty risks spinning down a
    /// fan that was running near maximum. An unwritable channel reports 0 and
    /// is refused on capability before the duty is ever used.
    fn probe_channel_state(
        &self,
        device_uid: &DeviceUID,
        channel_name: &str,
    ) -> Option<(bool, Option<RPM>, Duty)> {
        let device_lock = self.all_devices.get(device_uid)?;
        let device = device_lock.borrow();
        let pwm_writable = device
            .info
            .channels
            .get(channel_name)
            .and_then(ChannelInfo::speed_options)
            .is_some_and(|options| options.fixed_enabled);
        let channel_status = device
            .status_history
            .back()
            .and_then(|status| status.channels.iter().find(|c| c.name == channel_name));
        let baseline_rpm = channel_status.and_then(|channel| channel.rpm);
        if pwm_writable.not() {
            return Some((false, baseline_rpm, 0));
        }
        let current_duty = channel_status.and_then(|channel| channel.duty)?;
        Some((true, baseline_rpm, duty_from_percent(current_duty)))
    }

    /// The hottest reading on the machine, or `None` when nothing reports one.
    /// `hottest_temp` returns 0.0 in that case, which would read as a cool
    /// machine rather than an unknown one and quietly permit spinning a fan
    /// down.
    async fn hottest_probe_temp(&self) -> Option<f64> {
        let hottest = DiagnosisHost::hottest_temp(self, f64::MAX).await;
        if hottest.sensor.is_empty() {
            return None;
        }
...
#[async_trait::async_trait(?Send)]
impl ProbeHost for Engine {
    async fn gather_conditions(
        &self,
        device_uid: &DeviceUID,
        channel_name: &str,
    ) -> Option<ProbeConditions> {
        let (pwm_writable, baseline_rpm, current_duty) =
            self.probe_channel_state(device_uid, channel_name)?;
        let key: ChannelKey = (device_uid.clone(), channel_name.to_string());
        Some(ProbeConditions {
            pwm_writable,
            // A channel with no `fanN_input` reports no rpm at all, which is a
            // different refusal from one whose header is simply empty.
            has_tachometer: baseline_rpm.is_some(),
            baseline_rpm: baseline_rpm.unwrap_or(0),
            current_duty,
            alert_active: self.active_alert_blocking(&key).is_some(),
            temp_celsius: self.hottest_probe_temp().await,
        })
    }

    fn snapshot_setting(&self, device_uid: &DeviceUID, channel_name: &str) -> SettingsSnapshot {
        DiagnosisHost::snapshot_setting(self, device_uid, channel_name)
    }

    async fn restore_setting(&self, snapshot: &SettingsSnapshot) -> Result<()> {
        DiagnosisHost::restore_setting(self, snapshot).await
    }

    async fn enter_manual_control(&self, device_uid: &DeviceUID, channel_name: &str) -> Result<()> {
        DiagnosisHost::enter_manual_control(self, device_uid, channel_name).await
    }
    ...
    async fn write_duty(
        ...
        DiagnosisHost::write_raw_duty(self, device_uid, channel_name, duty).await
    }
    ...
    /// Asked of the repository that drives the device, never of hwmon by
    /// default: an hwmon `pwm_enable` read describes a control path the daemon
    /// is not using when the channel belongs to another repository.
    async fn manual_control_held(
        &self,
        device_uid: &DeviceUID,
        channel_name: &str,
    ) -> Option<bool> {
        let device_type = self.all_devices.get(device_uid)?.borrow().d_type;
        self.repos
            .get(&device_type)?
    ...
    async fn sleep_millis(&self, millis: u32) {
        DiagnosisHost::sleep_millis(self, millis).await;
    }
}
